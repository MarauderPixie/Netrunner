...
mittelrun_03 <- runner_liga_03 %>% group_by(Spieler) %>% summarize(Mittel = mean(Runs, na.rm = T))

figure(title = "Runs pro Spieler", ylab = "Runs", xlab = NULL, tools = NULL,
       xgrid = F, ygrid = F, legend_location = NULL, width = 800) %>% 
  ...
  ly_points(Spieler, Mittel, data = mittelrun_03, glyph = "x", color = "black", size = 20)
```
