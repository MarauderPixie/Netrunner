...
knitr::opts_chunk$set(fig.path = "assets/highscore-plots/")
...
as.data.frame.matrix(crossprod(table(score$Spiel, score$Spieler))) %>% 
  gather(Spieler, Spiele, Bjarne, Bodo, Boray, Falk, Jan,
         Johannes, Josh, `Josh R.`, Kai, Ole, Paul, Tobias, Ashkan) %>% 
  mutate(Gegner = rep(c("Ashkan", "Bjarne", "Bodo", "Boray", "Falk", 
                        "Jan", "Johannes", "Josh", "Josh R.", 
                        "Kai", "Ole", "Paul", "Tobias"), 13)) %>% 
...
## Hier waren mal fancy line charts
...
pref_runner  <- sjt.xtab(runner_score$Spieler, runner_score$Fraktion, title = "Runner",
                         show.exp = T, show.summary = F, emph.total = T,
...
chisq_sieg <- sjt.xtab(konzern_score$Fraktion, konzern_score$Siegbedingung,
                       title = "Konzernsiege", show.exp = T, emph.total = T,
                       no.output = T, use.viewer = F)
```
