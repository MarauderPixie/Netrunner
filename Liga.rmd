...
Außerdem sei noch angemerkt, dass für die weitere Analyse Jan und Jannis aus der Analyse genommen sind, da beide nur jeweils ein Spiel gespielt haben.

```{r filter}
liga         <- filter(liga, Spieler != "Jannis")
runner_liga  <-
konzern_liga <- 
```
...
```{r Spielerpräferenz}
pref_runner  <- sjt.xtab(runner_liga$Spieler, runner_liga$Fraktion, 
                         showExpected = T, showLegend = T, highlightTotal = T,
                         no.output = T, useViewer = F)

pref_konzern <- sjt.xtab(konzern_liga$Spieler, konzern_liga$Fraktion,
                         showExpected = T, showLegend = T, highlightTotal = T,
                         no.output = T, useViewer = F)
```
...
chisq_sieg <- sjt.xtab(sieg_tbl$Siegbedingung, sieg_tbl$Fraktion,
                       showExpected = T,  no.output = T, useViewer = F)
```
