...
runner_01  <- filter(runner, Saison == 1)
konzern_01 <- filter(konzern, Saison == 1)
```
...
```{r, Runs_Spieler}
ggplot(runner_01, aes(x = Spieler, y = Runs, color = Spieler)) +
  geom_point(position = "jitter") +
  stat_summary(fun.y = "mean", fun.ymax = "mean", fun.ymin = "mean", 
               colour = "black", size = 0.5, geom = "errorbar") +
  geom_hline(yintercept = mean(runner_01$Runs, na.rm = T), size = 1, color = "gray") +
  labs(x = NULL, y = "Runs") + 
