...
liga_01         <- filter(liga, Saison == 1)
runner_liga_01  <- filter(runner_liga, Saison == 1)
konzern_liga_01 <- filter(konzern_liga, Saison == 1)
```
...
format_table(games, 
             list(Bodo = yesno, Falk = yesno, Jannis = yesno,
                  Johannes = yesno, Kai = yesno, Tobias = yesno), 
             row.names = T, align = "c")
```
...
```{r, IDs}
  ggplot(liga_01, aes(x = Fraktion, fill = Fraktion)) +
...
```{r, Runs_Spieler}
ggplot(runner_liga_01, aes(x = Spieler, y = Runs, color = Spieler)) +
  geom_point(position = "jitter") +
  stat_summary(fun.y = "mean", fun.ymax = "mean", fun.ymin = "mean", 
               colour = "black", size = 0.5, geom = "errorbar") +
  geom_hline(yintercept = mean(runner_liga_01$Runs, na.rm = T), size = 1, color = "gray") +
  labs(x = NULL, y = "Runs") + 
...
mittelrun_01 <- runner_liga_01 %>% group_by(Spieler) %>% summarize(Mittel = mean(Runs))

figure(title = "Runs pro Spieler", ylab = "Runs", xlab = NULL, tools = NULL,
       xgrid = F, ygrid = F, legend_location = NULL) %>% 
  ly_lines(x = Spieler, y = mean(runner_liga_01$Runs), data = runner_liga_01,
           color = "gray", alpha = 0.8) %>% 
  ly_points(Spieler, Runs, data = runner_liga_01, 
            hover = "@Runs Runs als @ID gegen @Gegner_ID",
            glyph = "diamond", color = Fraktion, size = 15) %>% 
  ly_points(Spieler, Mittel, data = mittelrun_01, glyph = "x", color = Spieler, size = 20)
```
